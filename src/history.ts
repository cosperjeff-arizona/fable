...
export type ChangeEvent = { century: number; change: AppliedChange };
...
type ActiveBranch = {
  ...
  branch: Branch;
  stream: Stream;
  workingLexemes: Map<string, Lexeme>; // evolved through this branch's path so far
...
function applyChangeToBranch(active: ActiveBranch, change: SoundChange, century: number, protoLexicon: Lexicon): void {
  const newLexemes = new Map(active.workingLexemes);
  for (const [id, lexeme] of newLexemes) {
    const newWord = applyChange(lexeme.word, change);
    if (newWord !== lexeme.word) newLexemes.set(id, { ...lexeme, word: newWord });
  }
  active.workingLexemes = newLexemes;
  ...
  active.branch.events.push({ century, change: applied });
...
export function generateSimulation(config: Partial<Omit<SimConfig, 'seed'>> & { seed: number }): Simulation {
  ...
  const rootBranch: Branch = { id: 'root', start: 0, end: 0, events: [], paradigmNotes: [], children: [] };
  ...
  const makeChild = (parent: ActiveBranch, id: string, startCentury: number): ActiveBranch => ({
    id,
    start: startCentury,
    branch: { id, start: startCentury, end: startCentury, events: [], paradigmNotes: [], children: [] },
